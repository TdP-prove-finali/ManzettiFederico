package it.polito.tdp.GestioneDellaProduzionef;
/**
...
import it.polito.tdp.GestioneDellaProduzionef.model.BenchmarkOutput;
import it.polito.tdp.GestioneDellaProduzionef.model.DiagnosiU;
import it.polito.tdp.GestioneDellaProduzionef.model.Linea;
import it.polito.tdp.GestioneDellaProduzionef.model.Model;
import it.polito.tdp.GestioneDellaProduzionef.model.OptimizationResult;
import it.polito.tdp.GestioneDellaProduzionef.model.Parametro;
import it.polito.tdp.GestioneDellaProduzionef.model.SimResult;
import it.polito.tdp.GestioneDellaProduzionef.model.WorkStation;
...
public class FXMLController {
    ...
    void ottimizza(ActionEvent event) {
    	...
    	if(res.getLock() != null) {
			txtResult.appendText("Utilizzazione eccessiva su una o pi˘ ws della linea!!! Valori medi di utilizzazione per ws: \n");
    	...
    	for(WorkStation ws: res.getDiagnosiU().keySet()) {
			...
			for(DiagnosiU du:res.getDiagnosiU().get(ws)) {
				...
				if(du.getUtilizzazione()>1) {
					...
					//ad un certo punto dell'anno si blocca la linea, non Ë ammissibile
