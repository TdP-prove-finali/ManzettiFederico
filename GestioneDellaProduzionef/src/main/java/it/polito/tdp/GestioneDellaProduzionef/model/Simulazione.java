...
public class Simulazione {
	...
	public void init() {
		...
		for(WorkStation ws:linea.getListaWS()) {
			diagnosiU.put(ws, new ArrayList<DiagnosiU>());
		}
	...
	private boolean processDemand(Domanda d) {
		...
		for(WorkStation ws:linea.getListaWS()) {
			...
			//l'ordine di applicazioni delle variazioni Ë questo (supposizione)
			...
			if(u>1)
				flag = true; // perchË si blocca la linea
			
			
			//calcolo le prestazioni parziali della singola workstation con la formula pi˘ generale possibile
			...
			//bisogna ricalcolare la variabilit‡ in uscita (e quindi in entrata per la prossima ws) generata dalla workstation attuale
