...
import it.polito.tdp.GestioneDellaProduzionef.db.DAO;

public class Model {
	//per reperire i dati del database più agilmente
	private DAO dao;
	private WorkStationIDMAP map;
	
	//per gestire la linea di produzione
	private List<WorkStation> listaWS;
	private List<Domanda> domande;
	private Linea linea;
	
	//per analizzare le prestazioni della linea di produzione
	...
	//richiama la soluzione ottima del problema di ottimizzazione
	//direttamente dalla classe dedicata
	private OptimizationResult soluzioneOttima;

	public Model()
	{
	//unici parametri da inizializzare col nuovo model
	...
	public SimResult simula(String prodotto, Linea linea) {
		...
		//setto come valori dei parametri current i valori peggiori, quindi le variabilità/tempi/p di rilavorazione più
		//alti e distanza tra guasti/setup più bassi
		   ...
		   boolean flag2 = false;
			...
			for(WorkStation ws: res.getDiagnosiU().keySet()) {
				if(res.getDiagnosiU().get(ws).size() == 0) {
					...
					flag2 = true;  // c'Ë stato un blocco
					double sum=0;
